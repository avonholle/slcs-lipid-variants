...
# catch impossible configurations
  if (is.nan(max(R2)) | vg1>0 & abs(cov12)/sqrt(vg1)>1) {
...
# Non-centrality parameter!
  ...
  if (max(R2)>=1) {
    NCP[R2>=1]=1e10
    errorMsg="Warning: R2>=1 on the observed scale"
  }  
...
return(list(R2=R2,NCP=NCP,p=pchisq(NCP+1,1,lower=F),power=power,AUC=AUC,MSE=MSE,error=errorMsg))
}
